...
def game():
    ...
    enemy = pygame.Rect(WIDTH//2-40, 10, player_size, player_size)
    
    
    # Load and prepare the enemy image
    enemy_image = pygame.image.load('AllRoadsLeadToLinux.png')
    enemy_image = pygame.transform.scale(enemy_image, (player_size, player_size))
    enemy_rect = enemy_image.get_rect()
    enemy_rect.topleft = (WIDTH//2-40, 10)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.JOYBUTTONDOWN:
                if event.button == 5:  #trigger button
                    shoot = True
        ...
        if enemy_time > 0:
            ...
            if LorR_Enemy >= LorR_Calc:  
                enemy.x -= enemy_speed   #left
            elif LorR_Enemy < LorR_Calc: 
                enemy.x += enemy_speed   #right

        else:
            #Reset
            enemy_time = random.randint(45, 50)
            LorR_Enemy = enemy.x / 1543 * 100
            LorR_Calc = random.randint(0, 100) 
